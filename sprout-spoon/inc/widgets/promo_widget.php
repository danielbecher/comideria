...
class solopine_promo_widget extends WP_Widget {
	 ...
	 * Widget setup.
	 */
	function solopine_promo_widget() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'solopine_promo_widget', 'description' => __('Display a promo box with an image and link', 'sprout-spoon') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'solopine_promo_widget' );

		/* Create the widget. */
		parent::__construct( 'solopine_promo_widget', __('Sprout & Spoon: Promo Box', 'sprout-spoon'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		?>
			...
			<div class="promo-item" style="<?php if($image_url) : ?>background-image:url(<?php echo esc_url($image_url); ?>);<?php else : ?>background-color:#f4f4f4;<?php endif; ?> <?php if($height) : ?>height:<?php echo esc_html($height); ?>px;<?php else : ?>height:130px;<?php endif; ?>">
				<?php if($image_link) : ?><a <?php if($newtab) : ?>target="_blank"<?php endif; ?> href="<?php echo esc_url($image_link); ?>"></a><?php endif; ?>
				<?php if($heading) : ?>
	...
	function form( $instance ) {
		...
		<!-- Background URL -->
		<p>
			<label for="<?php echo $this->get_field_id( 'image_url' ); ?>"<?php esc_html_e( 'Background Image URL', 'sprout-spoon' ); ?>:</label>
			...
			<small><?php esc_html_e( 'Enter the background image URL you want to use. You can upload your background image via Media > Add New', 'sprout-spoon' ); ?></small>
		</p>
		
		<!-- Image Text -->
		<p>
			<label for="<?php echo $this->get_field_id( 'heading' ); ?>"><?php esc_html_e( 'Promo Text', 'sprout-spoon' ); ?>:</label>
		...
		<!-- Link -->
		<p>
			<label for="<?php echo $this->get_field_id( 'image_link' ); ?>"><?php esc_html_e( 'Promo Link', 'sprout-spoon' ); ?>:</label>
			...
			<small><?php esc_html_e( 'Enter a link you want the promo box to go to.', 'sprout-spoon' ); ?></small>
		</p>
		
		<!-- Height -->
		<p>
			<label for="<?php echo $this->get_field_id( 'height' ); ?>"><?php esc_html_e( 'Promo Box Height', 'sprout-spoon' ); ?>:</label><br>
		...
		<!-- Margin -->
		<p>
			<label for="<?php echo $this->get_field_id( 'margin' ); ?>"><?php esc_html_e( 'Promo Box Margin Bottom', 'sprout-spoon' ); ?>:</label><br>
		...
		<!-- New tab -->
		<p>
			<label for="<?php echo $this->get_field_id( 'newtab' ); ?>"><?php esc_html_e( 'Open link in a new tab?', 'sprout-spoon' ); ?></label>
