...
class solopine_facebook_widget extends WP_Widget {
	...
	function solopine_facebook_widget() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'solopine_facebook_widget', 'description' => __('A widget that displays a Facebook Like Box', 'sprout-spoon') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'solopine_facebook_widget' );

		/* Create the widget. */
		parent::__construct( 'solopine_facebook_widget', __('Sprout & Spoon: Facebook Like Box', 'sprout-spoon'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => 'Find us on Facebook', 'cover' => 'on', 'faces' => 'on', 'page_url' => '', 'stream' => false, 'border' => false);
		...
		<!-- Page url -->
		<p>
			<label for="<?php echo $this->get_field_id( 'page_url' ); ?>"><?php esc_html_e( 'Facebook Page URL', 'sprout-spoon' ); ?>:</label>
		...
		<!-- Faces -->
		<p>
			<label for="<?php echo $this->get_field_id( 'faces' ); ?>"><?php esc_html_e( 'Show Faces', 'sprout-spoon' ); ?>:</label>
		...
		<!-- Stream -->
		<p>
			<label for="<?php echo $this->get_field_id( 'stream' ); ?>"><?php esc_html_e( 'Show Stream', 'sprout-spoon' ); ?>:</label>
		...
		<!-- Cover -->
		<p>
			<label for="<?php echo $this->get_field_id( 'cover' ); ?>"><?php esc_html_e( 'Show Page Cover Image', 'sprout-spoon' ); ?>:</label>
